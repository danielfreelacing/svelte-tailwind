<script lang="ts">
	...
	const handleCategoryDelete = async () => {
		...
		if (result.data && result.data.categoryDelete?.isSuccess) {
			...
			menuStore.deleteCategory(categoryIdSelected);
			appStateStore.setCategoryIdSelected($menuStore.find( menu => menu.id === $appStateStore.menuIdSelected)!.categories[0].id)
			closeModalFunction()
		} else {
	...
	>
		<div class="w-full h-14 flex-col justify-start items-start gap-2 inline-flex">
			...
			<div class="text-zinc-900 mt-2 text-base font-normal font-['Poppins'] leading-normal">
				Are you sure you want to delete '{categoryName.charAt(0).toUpperCase() + categoryName.slice(1)}'?
			</div>
		</div>
