<script lang="ts">
	...
	export let extrasItem: ExtrasType;
	
	const dispatch = createEventDispatcher();

	$: editmode = false;

	let localName = extrasItem.name;
	let localPrice = extrasItem.price;
	$: extraEditName = localName;
	$: extraEditPrice = localPrice || null;
	$: extraEditActive = extrasItem.active
	function editExtra() {
		extrasItem = {
			id: extrasItem.id,
			name: extraEditName,
			price: extraEditPrice ? extraEditPrice : 0,
			active: extraEditActive
		};
		editmode = false;
		extraEditName = '';
		extraEditPrice = 0;
	...
	function removeExtra() {
		dispatch('removeExtra', {
			id: extrasItem.id, name: extraEditName, 
		});
	}
...
{#if !editmode}
	<div
		class="self-stretch w-full px-4 py-5 border-t border-gray-200 justify-start items-center gap-6 inline-flex"
	>
		<div class="grow shrink basis-0 h-6 justify-start items-center gap-8 flex">
			<div class="justify-start items-center gap-2 flex">
				...
				<div class="w-9 h-6 relative">
					<Switch bind:checked={extraEditActive} />
				</div>
			</div>
			<div
				class="grow shrink basis-0 text-zinc-900 text-base font-normal font-['Poppins'] leading-normal"
			>
				{extrasItem.name.charAt(0).toUpperCase() + extrasItem.name.slice(1)}
			</div>
...
{:else}
	<div
		class="w-full self-stretch px-4 py-3 border-t border-gray-200 justify-start items-center gap-4 inline-flex"
	>
