<script lang="ts">
	...
	export let merchantTypeList: MerchantType[];
	export let form;
	export let errors;
	export let constraints;
	export let merchantTypeSelected = merchantTypeList[0];
	export let avatarImageUrl;

	// $: console.log(" this is merchant data:", data)
	// $: console.log(" this is merchant data in form:", form)

	let merchantTypeIndex = 0;
	let typeComponentWidth = 0;

	$: merchantTypeSelected = merchantTypeList[merchantTypeIndex];
	...
	onMount(() => {
		if ($localAddressStore.copyAddresOver === true) {
			...
			console.log('localAddressStore in merchantform', $localAddressStore);
		}
	});
...
<div class="self-stretch flex-col justify-start items-start gap-6 sm:gap-10 flex">
	<div class=" h-20 justify-start items-center gap-4 inline-flex">
			...
			>
				{#if myAvatar !== undefined}
					<div class="w-20 h-20 relative">
						...
						>
							<img src={myAvatar} alt="merchant avatar" />
						</div>
					</div>
				{:else}
					<div
		...
		<div class="w-[552px] flex-col justify-start items-start gap-2 inline-flex">
			...
			<form
				...
				action="about-your-business?/merchantAvatarImageUrl"
	...
	<div class="w-[552px] flex-col justify-start items-start gap-4 inline-flex">
			...
			>
				<DropdownMenu.Root>
					<DropdownMenu.Trigger class="w-full">
						<div class="w-full basis-0 flex-row justify-between items-center inline-flex">
							<div class="flex flex-col w-full justify-between">
								...
								>
									{merchantTypeList[merchantTypeIndex].name.charAt(0).toUpperCase() +
										merchantTypeList[merchantTypeIndex].name.slice(1)}
								</div>
							</div>
					...
					<DropdownMenu.Content class="bg-white " style={`width: ${typeComponentWidth}px;`}>
						<DropdownMenu.Group>
							{#each merchantTypeList as merchantType, index}
								<DropdownMenu.Item>
									<button
										on:click={() => (merchantTypeIndex = index)}
										...
										>{merchantType.name.charAt(0).toUpperCase() +
											merchantType.name.slice(1)}</button
									></DropdownMenu.Item
								>
							{/each}
						</DropdownMenu.Group>
		...
		<div class="self-stretch h-14 flex-col justify-start items-start gap-1 flex">
			...
			>
				<div class="grow shrink basis-0 flex-col justify-start items-start gap-px inline-flex">
					{#if $form.merchantName !== '' && $form.merchantName !== undefined}
						<div class="flex flex-row justify-between w-full">
							...
							{#if $errors.merchantName}<span class="invalid">{$errors.merchantName}</span>{/if}
						</div>
					{/if}
					<input
						class="w-full {$errors.merchantName
						...
						bind:value={$form.merchantName}
						{...$constraints.merchantName}
					/>
				</div>
			</div>
		</div>
		...
		>
			<div class="w-full grow shrink basis-0 flex-row justify-start items-start gap-px inline-flex">
				<div
				...
				>
					{#if $form.description !== undefined && $form.description.length > 0}
						<div class="flex flex-row justify-between w-full">
							...
							{#if $errors.description}<div class="invalid text-xs">{$errors.description}</div>{/if}
						</div>
					{/if}
					<textarea
				...
				{#if $form.description !== undefined && $form.description.length > 0}
					<div class="basis-[6px] self-stretch justify-end items-start flex">
						<div class="text-zinc-500 text-xs font-normal font-['Poppins'] leading-tight">
